...
func (s *session) Send(ctx context.Context, prompt string) (xagent.Stream, error) {
	...
	if s.closed {
		s.mu.Unlock()
		return nil, &xagent.AgentError{Code: xagent.ErrInvalidConfig, Agent: "opencode", Message: "session closed"}
	}
	sid := s.sessionID
	fork := s.forceFork
	cfg := s.cfg
	...
	env := s.agent.baseEnv(cfg.Env)
	env["OPENCODE_CONFIG_CONTENT"] = buildConfigContent(cfg)

	fullArgs := append([]string{s.agent.command()}, buildRunArgs(cfg, prompt, sid, fork)...)
	...
	return xagent.NewProcessStream(proc, scanner, parseSSELine,
